...
def api_add_topic(
    ...
    with get_conn() as conn, conn.cursor() as cur:
        cur.execute(
            ...
            (author_id_val, title.strip(), direction_val),
        )
        if cur.fetchone():
        ...
        cur.execute(
            ...
            RETURNING id
            ''', (author_id_val, title.strip(), (description or None), (expected_outcomes or None), (required_skills or None), direction_val, seeking_role),
        )
        tid = cur.fetchone()[0]
        conn.commit()
